...
class ThreadPoolOld(threading.Thread):
	"""Threaded Url Grab"""
	def __init__(self, queue):
		threading.Thread.__init__(self)
		self.queue = queue
		self.test=self.queue.qsize()
		#self.start()
	def run(self):
		...
		while self.queue.qsize()>0: #not self.queue.full():
			...
			func(*args)
			self.test = self.queue.qsize()
			print 'qsize', self.test
			print 'full', self.queue.full()
