...
class psql(threading.Thread):
	...
	def get_q_rx(self, q):
		...
		if q.strip().upper().startswith('TRUNCATE'):
			return (r'(TRUNCATE TABLE and COMMIT TRANSACTION|Time\: \d+\.\d+ ms)',)	
		if q.strip().upper().startswith('SELECT COUNT(*)'):
			return (r'(.*)',)		
		if q.strip().upper().startswith('SELECT'):
			return (r'(\(\d+ rows?\)|Time\: \d+\.\d+ ms|ERROR|Server closed the connection unexpectedly\.)',)
		if q.strip().upper().startswith('UNLOAD') or q.strip().upper().startswith('COPY') or q.strip().upper().startswith('--UNLOAD') or q.strip().upper().startswith('--COPY'):
			return (r'(\(\d+ rows?\)|Time\: \d+\.\d+ ms|COPY|UNLOAD)',)
		if q.strip().upper().startswith('DROP') or q.strip().upper().startswith('CREATE'):
			return (r'(.*)',)			
		return (r'(\(\d+ rows?\)|Time\: \d+\.\d+ ms|\w+)',)
...
SELECT etlflow,worker,elapsed_sec,etlflow_sec,worker_sec, num_rows,count_star,created_dt, log_id from public.smart_test_log order by log_id  desc, etlflow,worker desc
		...
		#print ins		
		return (t_r, ins)	
	def psql_dml(self, etl_object, logger):
		...
		if 0:
			...
			if self.p_if('IF_TOSS_CACHE'):
				(out,status,err) = self.do_pa_query(self._default_login, "xpx 'toss_cache';\n",0,('.*',)) 
			
			logger.info('#### START of PADB dml %s.' % etl_object['name'])
			regexpt= self.get_q_rx(template)
			a_t = datetime.datetime.now()
			(out,status,err) = self.do_pa_query(self._default_login, template,0,regexpt)
			...
			if status==0:
				self.save_pa_log(etl_object, template, out,status, worker_sec,logger)
			else:
				self.save_pa_log(etl_object, template, err,status, worker_sec,logger)
		...
		return status
	def save_pa_log(self,etl_object, template, out,status, worker_sec=None,logger=None):
		if not logger:
			logger=self._logger
		...
		if not self.p_if('IF_ASYNC_LOG'):
			...
			ins_ok = 'INSERT 0 1'
			(out,status,err) = self.do_pa_query(self._default_login, ins,0,('(%s)' % ins_ok,))
			if status>0:
...
/
	...
	def ora2pa_pipe(self, etl_object, logger):
		""" Runs copy using SQL*Plus and SQL*Loader"""
		self.set_params(etl_object, logger)
		(tmpl_batch, status) = self.get_tmpl_batch(etl_object, logger)
				...
				if 1:
					...
					if 1:
						...
						if self.p_if('IF_TRUNCATE'):
							...
							a_t = datetime.datetime.now()
							(r, status,err) = self.do_pa_query(to_db,tmpl,0,rx)
							worker_sec = datetime.datetime.now()-a_t
							logger.info('#### END truncate of %s.' % etl_object['name'])						
							self.save_pa_log(etl_object, tmpl, r,status, worker_sec, logger)
						if status==0:
							(r, status) = self.do_ora2pa_load(etl_object, from_db,  to_db, from_tab,to_tab)
	...
	def file2pa_load(self, etl_object, logger):
		""" Runs copy using SQL*Plus and SQL*Loader"""
		self.set_params(etl_object, logger)
		(tmpl_batch, status) = self.get_tmpl_batch(etl_object, logger)
				...
				if 1:
					...
					if 1:
						...
						if self.p_if('IF_TRUNCATE'):
							...
							tmpl= 'TRUNCATE %s;\n' % to_tab[1]
							rx= self.get_q_rx(tmpl)
							#(r, status) = self.do_pa_query(to_db,tmpl,0,rx)
							logger.info('#### END truncate of %s.' % etl_object['name'])						
							#self.save_pa_log(etl_object, tmpl, r,status, logger)
						if status==0:
							(r, status) = self.do_file2pa_copy(etl_object, from_db,  to_db, from_tab,to_tab)
	...
	def do_file2pa_copy(self, etl_object,from_db, to_db, from_t, to_t):
		f = ""
		out=[]
		err=[]
		...
		if not status:
			...
			if 1:
				self._logger.info('Starting UNCOMPRESSED file2psql COPY')
				
				#p2 = Popen([ "/home/paraccel/padb/rel/bin/psql", "DEPOT"], stdin=p1.stdout, stdout=PIPE , stderr=PIPE)

				#p2 = Popen([ "/home/paraccel/padb/rel/bin/psql", "DEPOT", "-c","\nselect count(*) cnt_before from TRD_VOL_UNION_1;\n\n copy TRD_VOL_UNION_1 from '%s' delimiter '|' dateformat 'YYYY/MM/DD' timeformat 'YYYY/MM/DD HH:MI:SS';\nselect count(*) cnt_after from TRD_VOL_UNION_1;\n" % srcf], stdin=PIPE, stdout=PIPE , stderr=PIPE
				#cmd="psql  -c \"copy %s from '%s' delimiter '|' dateformat 'YYYY/MM/DD' timeformat 'YYYY/MM/DD HH:MI:SS';\" & /home/iz31541/Oracle11/sqlplus -s ab95022/Aug106@gmail.com @/home/paraccel/data_spooler/%s>%s" % (to_t[1],gzfn,sqfn,gzfn)
				#cmd="/home/iz31541/Oracle11/sqlplus -s ab95022/Aug106@gmail.com @/home/paraccel/data_spooler/%s>%s & psql  -a -c \"copy %s from '%s' delimiter '|' dateformat 'YYYY/MM/DD' timeformat 'YYYY/MM/DD HH:MI:SS';\"" % (sqfn,gzfn,to_t[1],gzfn)				
				#print cmd
				#os.system( cmd)
				#cmd="/home/iz31541/Oracle11/sqlplus -s ab95022/Aug106@gmail.com @/home/paraccel/data_spooler/%s>%s &" % (sqfn,gzfn) 
				#print cmd
				#os.system( cmd)
				#p2 = Popen([ "/home/iz31541/Oracle11/sqlplus", "-s", "ab95022/Aug106@gmail.com",   "@/home/paraccel/data_spooler/tmp/logs/PIPED_TEST/20110911_183256/sql/CSMARTVOL.TRD_VOL_UNION_1.SPOOL.CSMARTVOL.TRD_VOL_UNION_1.sql"], stdin=PIPE, stdout=log , stderr=PIPE
				#p2 = Popen([ "/home/paraccel/padb/rel/bin/psql", "DEPOT"], stdin=p1.stdout, stdout=PIPE , stderr=PIPE)
				#)
				#status=p2.wait()
				if 1:
					p3 = Popen([ "/home/paraccel/padb/rel/bin/psql", self._padb], stdin=p1.stdout, stdout=PIPE , stderr=PIPE)
					status =0
					out=[]
					err=[]
					if 1:
		...
		#a_t = datetime.datetime.now()
		worker_sec = datetime.datetime.now()-a_t
		self.save_pa_log(etl_object, q, out,status,worker_sec)
		return (out,status)
	def do_ora2pa_load(self, etl_object,from_db, to_db, from_t, to_t):
		f = ""
		out=[]
		err=[]
		self.confirm(len(from_db)>0, 'Source login is not set.')
		...
		if not status:
			...
			grp=None
			if 1:
				self._logger.info('Starting UNCOMPRESSED psql COPY')
	...
	def sql_echo_loader_fixed(self, etl_object, logger):
		...
		return 0		
	def do_pa_query(self, login, query, analyze=1, regexpt=None, grp=None):
		f = ""
		out=[]
		err=[]
		ora_err =False
		...
		if 1:			
			...
			login= login.split(' ')
			#print login
			p2 = Popen([ 'psql', "-U", 'paraccel', "-d", self._padb,'-a'], stdin=p1.stdout, stdout=PIPE, stderr=PIPE)
			output=' '
			while output:
				output = p2.stdout.readline()
				#err.append(output)
			...
			if status==0:
				if len(err):
					status=2
			print 'do_pa_query status= ', status
		#sys.exit(1)
		#print 'end'
...
(%s)""" % (to_tab, string.join(r_int,',\n'))
	...
	def get_pa_login(self, connector):
		confirm(connector.has_key('type'),'PADB connector needs type attribute to be set.')
...
""" % (t[1],t[0], ci)
	...
	def do_query(self, login, query, analyze=1, regexp=None, grp=None):
		...
		errpos=-1
		#errreg=re.compile(r'.*(ERROR).*')
		self.confirm(len(login)>0, 'Default login is not set.')
