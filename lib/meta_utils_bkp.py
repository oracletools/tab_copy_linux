...
""" % (qry[0:16000].replace("'","''"),qry[16000:32000].replace("'","''"),qry[32000:48000].replace("'","''"),qry[48000:64000].replace("'","''"),qry[64000:80000].replace("'","''"))
			regexp=re.compile(r'([\w\_\:\(\)\d]+)')
			#regexp=re.compile(r'(.*)')
			#print(q);
			#sys.exit(1)
...
""" % (t[1],t[0], ci)
	...
	def get_prt_set(self, to_db, to_tab, pt_mask):
		...
		%s""" % (200,"SELECT PARTITION_NAME FROM ALL_TAB_PARTITIONS WHERE TABLE_OWNER='%s' AND TABLE_NAME = '%s' ORDER BY 1;\n" % (tuple(to_tab)))
		#pprint(pq)
	...
	def get_tab_shards(self,nosh,from_db,from_t,options):
		...
		#sys.exit(1)
		q="""
		select /*+parallel */ distinct nt||'||'||min(id)||'||'||max(id)||'||'||'%s'  
      from ( 
